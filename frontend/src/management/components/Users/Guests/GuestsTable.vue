    ...
    >
      <template #header>
        <div class="flex justify-between items-center">
          ...
          <div>
            <h2 class="text-xl font-semibold text-center">Guests Customers Table</h2>
          </div>
        </div>
      </template>
      <template #empty>
      ...
      <template #loading>Loading guests data. Please wait.</template>
      <Column header="User ID" style="min-width: 12rem">
        <template #body="{ data }">
          <button v-tooltip.bottom="'Copy id'" @click="copyToClipboard(data.guest_id_original, data.guest_id_original)">
            {{ data.guest_id }}
            <span v-if="copiedState.get(data.guest_id_original) ?? false" class="text-xs text-green-500">Copied!</span>
          </button>
        </template>
      </Column>
      ...
      <Column field="phone_mask" header="Phone Number" style="min-width: 12rem">
        <template #body="{ data }">
          <button v-tooltip.bottom="'Copy number'"
                  @click="copyToClipboard(data.phone_original, `phone_${data.guest_id_original}`)">
            {{ data.phone_mask }}
            <span v-if="copiedState.get(`phone_${data.guest_id_original}`) ?? false" class="text-xs text-green-500">Copied!</span>
          </button>
        </template>
      </Column>
      <Column
...
async function fetchGuests(pageIndex) {
  if (cache.has(pageIndex)) {
    guests.value = cache.get(pageIndex);
    return;
  }
  ...
  await RegularCustomersService.getGuests(params)
      .then((response) => {
        if (response?.data.meta.total === 0) {
          guests.value = response;
        } else {
          guests.value = response.data.data?.map((item) => ({
            guest_id_original: item.id,
            ...item.attributes
          })) || [];
          totalRecords.value = response.data.meta.total;
          cache.set(pageIndex, guests.value);
        }
      })
      .catch((e) => {
      })
