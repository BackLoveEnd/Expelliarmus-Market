...
const fetchCoupons = async (pageIndex) => {
  ...
  await ShopCouponService.getCoupons(`/users/coupons/personal?limit=${limit.value}&offset=${pageIndex * limit.value}`)
      .then((response) => {
...
<template>
  ...
  >
    ...
    <Column field="user_email" header="Email" style="width: 30%; padding-left: 1rem">
      <template #body="slotProps">
        <span class="font-bold">{{ slotProps.data.user_email }}</span>
      </template>
    </Column>
    <Column field="discount" header="Discount %" style="width: 10%; padding-left: 1rem"/>
    <Column field="type" header="Type" style="width: 20%">
      <template #body="slotProps">
        <span class="text-violet-500">{{ slotProps.data.type }}</span>
      </template>
    </Column>
