  ...
  <div v-if="isFetching === false && userOrders.length === 0">
    <main class="grid place-items-center">
      <div class="text-center">
        <p class="mt-6 text-lg font-medium text-pretty text-gray-500 sm:text-xl/8">Delivered Orders not found.</p>
      </div>
    </main>
  </div>
...
async function getPendingOrders(pageIndex) {
  ...
  await ManagementOrdersService.getOrders(`/orders/delivered?limit=${limit.value}&offset=${pageIndex * offset.value}${searchQuery}`)
      .then((response) => {
